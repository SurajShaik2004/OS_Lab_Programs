...
// The process with highest number is having highest priority.
...
void CompletionTime(struct node arr[], int size)
{
    int curtime = 0, j = 0, i = 0;
    struct node *p;
    struct node temp = {0, 0, 0, 0, -1};
    int completed = 0;

    printf("\nGant Chart :");
    printf("\n_________________________________________________________________________________________");
    printf("\n|00|");
    // Among the current available processes selecting the one with Highest Priority;
    ...
    while (completed != (size + 1) && j <= size)
    {
        i = 0;
        if (arr[j].AT <= curtime)
        {
            p = &temp;
            // iterating through all the available processes and finding the High priority process out of them.
            while (i <= size && arr[i].AT <= curtime)
            {
                if ((arr[i].priority > p->priority) && arr[i].CT == 0)
                {
                    p = &arr[i];
...
void print(struct node arr[], int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        printf("|  %02d  | %02d | %02d |    %02d    |\n", arr[i].pid, arr[i].AT, arr[i].BT, arr[i].priority);
    }
    printf("_________________________\n");
}
...
void printall(struct node arr[], int size)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        avgtat += tat;
        avgwt += wt;
        printf("|  %02d  | %02d | %02d |    %02d    | %02d | %02d | %02d |\n", arr[i].pid, arr[i].AT, arr[i].BT, arr[i].priority, arr[i].CT, tat, wt);
    }
